#!/bin/bash
# Impulso Sticker Builder Professional — launcher para macOS/Linux.
#
...
# Impulso) — hace falta un servidor local mínimo. Este script NO envía nada
...
# disponibles la próxima vez que abras Impulso (incluida una versión
...
if ! command -v python3 >/dev/null 2>&1; then
  if command -v python >/dev/null 2>&1; then
    ...
    echo "Impulso necesita Python (ya instalado en la mayoría de Mac/Linux) para abrirse."
...
"$PYTHON" -m http.server "$PORT" >/tmp/impulso-sticker-builder.log 2>&1 &
SERVER_PID=$!
...
echo "Impulso Sticker Builder está corriendo en $URL — NO cierres esta ventana mientras lo uses."
...
echo "Deja esta ventana abierta mientras usas Impulso. Ciérrala para salir."
wait "$SERVER_PID"
