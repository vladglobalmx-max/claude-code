...
| **Asset Library** | Sticker Builder (Epic 1) ya permite subir imágenes PNG/SVG desde la UI, pero sin una Asset Library real detrás: el binario se embebe como data URL directamente en el `ImageObject` (`customProperties`), con un `assetId` sintético no respaldado por ningún registro — sin deduplicación, sin gestión/reutilización entre proyectos (ver [`../adr/0010-sticker-creation-experience.md`](../adr/0010-sticker-creation-experience.md)). | La etapa Beta lo requiera explícitamente (ver `04-Roadmap.md`) — migrar de data URL embebido a una Asset Library real es un cambio de dato, no de arquitectura (el punto de extensión `resolveAssetSource` del Renderer no cambia). |
