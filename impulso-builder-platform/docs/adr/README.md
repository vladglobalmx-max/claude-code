...
| [0026](0026-commercial-platform-boundaries.md) | Fase 4.1 | Commercial Platform Boundaries: vocabulario Module/Feature/Commercial Product/Entitlement/License/Channel, arquitectura de 11 capas con dependencia unidireccional (el dominio creativo nunca depende de la capa comercial), `@impulso/commercial-schema` como único paquete nuevo (contrato de datos puro, sin lógica ni proveedor) |
| [0027](0027-product-manifest.md) | Fase 4.1 | Commercial Product Manifest: formato versionado `commercial-product.json`, validación Zod con fallback seguro, `licensingMode` como enum abierto, decisión deliberada de no fijar todavía la mecánica de entrega del archivo real |
| [0028](0028-entitlements-and-licensing.md) | Fase 4.1 | Entitlements & Licensing V1: `licensingMode: "delivery-only"` (Gumroad gatea la entrega, la app no valida nada técnicamente) como la única opción compatible con "sin backend, sin DRM agresivo, sin bloquear compradores legítimos"; modelo de `Entitlement` diseñado como contrato para Fase 4.3, sin emisor/consumidor real todavía |
| [0029](0029-distribution-strategy.md) | Fase 4.1 | Distribution Strategy: Gumroad como canal de checkout/entrega inicial, Bookfluence como storefront/marketing (nunca el motor interno de los Builders), interfaz `CommerceAdapter` neutral de proveedor, separación dura marketing/commerce/identity/application runtime |
