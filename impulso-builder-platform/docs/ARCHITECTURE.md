...
Nesting irregular/optimización automática de desperdicio, tiling de gran formato, integración con RIP/plotter, CMYK/perfiles ICC/Spot Colors certificados (todo lo anterior, Print Engine/Epic 9), cuentas/auth productiva, sincronización remota, colaboración en tiempo real, marketplace, un segundo módulo real, arquitectura de plugins abierta a terceros, backend de cualquier tipo, pagos/checkout, gating real de capabilities — todos deliberadamente diferidos hasta que exista una necesidad de negocio concreta, no por limitación técnica. Epic 9 (Professional Print Engine) está completa y cerrada (Fases 9.1-9.5, ver `docs/platform/TRACEABILITY_MATRIX_EPIC9.md`). Fase 4.1 (Commercial Platform Architecture & Product Packaging) está completa — diseño y prototipo de contrato de datos únicamente, sin ninguna épica de implementación comercial autorizada todavía (ver sección 9 y `docs/platform/COMMERCIAL_PLATFORM_ROADMAP.md`). Ver [`product/PRODUCT_BACKLOG.md`](product/PRODUCT_BACKLOG.md) para estas capacidades evaluadas con valor/prioridad/dependencias/complejidad, y [`WHAT_SHOULD_WE_BUILD_NEXT.md`](../WHAT_SHOULD_WE_BUILD_NEXT.md) para la recomendación de la próxima épica.

## 9. La capa comercial (Fase 4.1 — diseño y contrato de datos, sin producto todavía)
...
De las 11 capas, solo la 3 (Commercial Product Manifest) tiene código real hoy: `packages/commercial-schema` — tipos + esquemas Zod puros para `CommercialProduct`, `ProductManifest`, `Capability`, `Entitlement`, sin lógica de evaluación, sin I/O, sin conocimiento de ningún proveedor de comercio. Es deliberadamente un paquete de foundation, análogo en espíritu a `document-schema` mismo pero para el dominio comercial — ver [ADR-0027](adr/0027-product-manifest.md)/[ADR-0028](adr/0028-entitlements-and-licensing.md)/[ADR-0029](adr/0029-distribution-strategy.md) y `docs/platform/COMMERCIAL_PRODUCT_MODEL.md`/`CAPABILITY_MODEL.md`/`LICENSING_THREAT_MODEL.md`/`V1_COMMERCIAL_RECOMMENDATION.md`/`COMMERCIAL_PLATFORM_ROADMAP.md`/`COST_MODEL.md` para el modelo completo. Ningún comportamiento visible de `apps/sticker-builder` cambió en Fase 4.1 — hoy, como siempre, toda capacidad del editor está activa sin excepción.
