...
## Fase 4 — Commercial Platform 🔶 En progreso (Fase 4.1 y 4.2 completas)
...
- **Fase 4.1 — Commercial Platform Architecture & Product Packaging ✅ Completa**: arquitectura de 11 capas con dependencia unidireccional (ver [ADR-0026](../adr/0026-commercial-platform-boundaries.md)), modelo `CommercialProduct`/`ProductManifest`/`Capability`/`Entitlement` (ADR-0027/0028), Gumroad como canal de checkout/entrega inicial y Bookfluence como storefront/marketing (ADR-0029), threat model inicial, recomendación V1 concreta (pago único, sin cuenta obligatoria, sin backend, `licensingMode: "delivery-only"`), roadmap de fases 4.2-4.8, y el primer paquete de foundation (`@impulso/commercial-schema`, contrato de datos puro, 37 tests, 100% cobertura) — sin ningún cambio de comportamiento en el producto real.
- **Fase 4.2 — Commercial MVP: First Sellable Sticker Builder ✅ Completa**: "Impulso Sticker Builder Professional" v1.0.0 convertido en un producto real, empaquetable y listo para vender vía Gumroad. `@impulso/capabilities` (primera implementación real de `CapabilityProvider`), manifest real conectado (`edition: "professional"`, `licensingMode: "delivery-only"`), `pnpm build:commercial` (paquete reproducible + ZIP + checksum SHA-256, harnesses de Epic 9 excluidos por construcción), launchers de un clic (Windows/macOS/Linux, puerto fijo 4173) verificados offline en Chromium real, backup/portabilidad de proyectos (exportar/importar), bienvenida de primera ejecución + "Estado comercial" honesto, documentación legal (EULA/privacidad/licencias de terceros) y documentación completa del comprador en español, plan de publicación en Gumroad (copy/precio/checklist) listo sin publicar todavía. Ver `docs/platform/COMMERCIAL_BUILD_GUIDE.md`/`PACKAGING_GUIDE.md`/`GUMROAD_LAUNCH_PLAN.md`.
- **Principio para autorizar 4.3+ (adoptado tras cerrar 4.2)**: el objetivo de negocio inmediato es "vender la primera copia", no maximizar infraestructura comercial. Cualquier sub-fase futura se evalúa contra "¿esto ayuda a vender/entregar/usar la primera copia?" — si la respuesta es no, se documenta como idea diferida (backlog) en vez de construirse. Entitlements/licensing técnico, commerce-adapters (integración real con la API de Gumroad), cuentas/auth, cloud sync, telemetría, panel de administración, marketplace y DRM son explícitamente **B/C** bajo este criterio (útiles con clientes reales o como arquitectura futura, no bloqueantes para la primera venta) — permanecen en backlog, no se construyen sin evidencia de demanda real. Ver el reporte de revisión YAGNI de Fase 4.2 para la clasificación completa tarea por tarea.
- Ver `docs/platform/COMMERCIAL_PLATFORM_ROADMAP.md` para el detalle de cada sub-fase siguiente (4.3+), cada una con su propia autorización.
...
- **Facturación**, si el negocio define una propuesta de precios validada — V1 usa pago único vía Gumroad, sin facturación propia.
