> **Documento archivado (Epic 6 — Platform Consolidation, 2026-07-19).** Este es el diseño ORIGINAL de Fase 0 — escrito antes de que existiera una sola línea de código de producto. Varias decisiones tecnológicas concretas descritas aquí (React, react-konva, Zustand, Tailwind, Radix UI, `idb`) **nunca se implementaron**: la plataforma real se construyó con TypeScript plano (sin framework de UI), Konva directo (sin wrapper de React), CSS/DOM ad-hoc, e IndexedDB nativo. Los principios estructurales de fondo (Document Schema → Engine → Renderer, sin dependencia de render en el núcleo) sí se mantuvieron y se verifican activamente. Se conserva como registro histórico de la intención original — el estado real y actual de la plataforma vive en [`../ARCHITECTURE.md`](../ARCHITECTURE.md).
>
> Ver [`docs/platform/STATE_001.md`](../platform/STATE_001.md) para la auditoría completa que motivó este reemplazo.
