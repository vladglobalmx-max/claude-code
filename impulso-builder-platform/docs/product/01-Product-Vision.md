...
Impulso es una **plataforma de creación de productos visuales**, construida como un núcleo reutilizable (**Impulso Engine**) del que se alimentan **módulos** independientes — cada uno resolviendo un tipo de producto distinto (stickers, planners, coloring books, y los que vengan después) sin reescribir el editor desde cero cada vez.

**Impulso Sticker Builder** es el primer módulo: un editor visual enfocado en un objetivo único — llevar a alguien desde "tengo una idea de sticker" hasta "tengo un archivo listo para imprenta" (PNG/SVG/PDF con línea de corte y sangrado), sin fricción y sin necesitar software de diseño profesional.

Impulso no es "un editor de canvas más". Es la apuesta de que la mayoría de los "editores de producto visual" (stickers, planners, coloring books, tarjetas, etiquetas...) comparten el mismo problema de fondo — capas, formas, selección, transformación, exportación, historial — y ese problema merece resolverse **una sola vez**, bien, y reutilizarse.
...
- **No es una plataforma de plugins públicos ni un marketplace** — la arquitectura de módulos/plugins existe para que Impulso mismo pueda crecer (Sticker Builder, Planner Builder, Coloring Book Builder...), no (todavía) para que terceros publiquen extensiones.
...
- **Un núcleo, múltiples productos** — cada módulo nuevo (Planner Builder, Coloring Book Builder...) hereda selección, transformación, historial y exportación ya resueltos, en vez de reconstruirlos.
...
2. Validar que el núcleo (Impulso Engine) generaliza a un segundo módulo real (Planner Builder o Coloring Book Builder) sin reescritura — la prueba definitiva de que la arquitectura modular no es solo una aspiración.
