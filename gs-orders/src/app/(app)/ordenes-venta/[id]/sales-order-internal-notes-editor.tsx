...
import { updateSalesOrderInternalNotes } from "../actions";

/**
 * "Nota interna" — editable en cualquier status (ver
 * updateSalesOrderInternalNotes en actions.ts): internal_notes está
 * explícitamente fuera del congelamiento comercial de
 * trg_sales_order_status_transition (0067).
 */
export function SalesOrderInternalNotesEditor({
  salesOrderId,
  initialNotes,
  canWrite,
}: {
  ...
  function handleSave() {
    startTransition(async () => {
      const result = await updateSalesOrderInternalNotes(salesOrderId, value);
      if (result?.error) {
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-xs text-ink-faint">
        Visible solo para tu equipo — no forma parte del contenido comercial de la Sales Order.
      </p>
