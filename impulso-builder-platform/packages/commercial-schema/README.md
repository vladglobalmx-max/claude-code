...
> Contrato de datos de la Commercial Platform, nacido en Fase 4.1 (Commercial Platform Architecture & Product Packaging) tras el cierre de Epic 9. Ver [ADR-0026](../../docs/archive/adr-commercial/0026-commercial-platform-boundaries.md), [ADR-0027](../../docs/archive/adr-commercial/0027-product-manifest.md) y [ADR-0028](../../docs/archive/adr-commercial/0028-entitlements-and-licensing.md) (archivados tras `THOREN_PRODUCT_DIRECTION.md`, escenario D — el motor sigue vigente como componente interno, ver `docs/product/THOREN_STICKER_BUILDER_COMPONENT.md`).
...
- **No hace:** no evalúa si una capability está concedida, no emite ni valida licencias, no llama a ningún proveedor externo. `CapabilityProvider` es solo una interfaz — su implementación real (empezando por un `OpenCapabilityProvider` que concede todo, ver `docs/archive/sticker-builder/commercial-platform/CAPABILITY_MODEL.md`, archivado) es trabajo de Fase 4.2.
...
Es la única excepción documentada al principio "no crear un paquete sin consumidor real" (ver `docs/ENGINEERING_STANDARDS.md` §"Principios de producto" y ADR-0026, archivado) — permitida explícitamente por el enunciado de Fase 4.1 como "prototipo técnico mínimo... para validar arquitectura". Es deliberadamente pequeño (solo tipos + Zod) para minimizar ese riesgo aceptado.
