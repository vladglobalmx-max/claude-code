...
>
> Para la narrativa de POR QUÉ cada pieza se construyó así, ver los [ADRs](adr/) (uno por Foundation/Epic) y su [índice de lectura rápida](product/06-Architecture-Decisions.md). Para el mapa de PRODUCTO/MÓDULOS vigente (qué existe vs. qué está planeado, sin detalle técnico) ver [`product/THOREN_TECHNICAL_ARCHITECTURE.md`](product/THOREN_TECHNICAL_ARCHITECTURE.md) — este documento es su compañero técnico, no un duplicado. (El `03-Architecture-Map.md` de la plataforma pre-reinvención, que cumplía esta misma función antes de THÖREN 2.0, queda archivado en [`archive/pre-reinvention-platform/`](archive/pre-reinvention-platform/03-Architecture-Map.md).)
...
No existen (todavía) `packages/ui`, `packages/config` ni una carpeta `plugin/` dentro de la app — mencionados en el diseño original de Fase 0 pero nunca construidos porque no hay, hasta hoy, un segundo módulo real que los justifique (ver "Simplicidad" en [`ENGINEERING_STANDARDS.md`](ENGINEERING_STANDARDS.md#principios-de-producto-fusionado-desde-docsproduct02-product-principlesmd-consolidación-documental-2026-07-31)).
