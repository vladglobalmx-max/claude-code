...
### Agregado
- `commercial-product.json`: manifest real y checked-in (`productId: "impulso-sticker-builder"`, `edition: "professional"`, `channel: "gumroad"`, `licensingMode: "delivery-only"`, catálogo real de capabilities). Validado en build-time (`pnpm validate:manifest`, `scripts/validate-commercial-manifest.mjs`) — política de fallo build-time obligatoria, ver ADR-0027 (enmienda Fase 4.2).
- `src/commercialManifest.ts`: `getCommercialManifest()` — import JSON estático (sin fetch en runtime), fallback abierto solo en desarrollo local ante un manifest inválido (nunca en el build comercial, que ya habría fallado la validación).
- `@impulso/capabilities` (nuevo paquete) conectado como primer `CapabilityProvider` real — ver `docs/platform/CAPABILITY_PROVIDER_GUIDE.md`.
- `src/projectBackup.ts`: backup/portabilidad real de proyectos — `serializeProjectBackup`/`importProjectBackup`, envelope JSON con assets embebidos en base64 (`FileReader`, no `Blob.arrayBuffer()`/`File.text()` — ninguno implementado en jsdom, confirmado por tests reales). Botones "Exportar respaldo"/"Importar proyecto" en la Workspace.
- `src/welcomeDialog.ts`: bienvenida de primera ejecución (gateada por `localStorage`, nunca repetida) + "Estado comercial" honesto visible en la Workspace (nunca dice "activado" — no existe activación real en `delivery-only`).
- `scripts/build-commercial.mjs` + `vite.commercial.config.ts`: comando `pnpm build:commercial` — produce un paquete reproducible (verificado: mismo commit → mismo SHA-256 del bundle de JS) + `.zip` + `CHECKSUMS.sha256`. Excluye por construcción los harnesses de Epic 9 (`print-engine-harness.html`/`print-preview-harness.html`) y escanea el paquete final contra archivos de desarrollo/secretos. Ver `docs/platform/COMMERCIAL_BUILD_GUIDE.md`.
- `commercial-assets/`: launchers de un clic para Windows/macOS/Linux (servidor HTTP local en el puerto fijo 4173 — evita las restricciones de `file://`, preserva IndexedDB entre actualizaciones), documentos legales (EULA, privacidad, licencias de terceros), documentación completa del comprador en español (7 guías + `LEEME-PRIMERO.md`), notas de versión.
- `docs/platform/OFFLINE_DISTRIBUTION_GUIDE.md`, `COMMERCIAL_SECURITY_CHECKLIST.md`, `COMMERCIAL_BUILD_GUIDE.md`, `PRODUCT_MANIFEST_GUIDE.md`, `CAPABILITY_PROVIDER_GUIDE.md`, `PACKAGING_GUIDE.md`, `RELEASE_CHECKLIST.md`, `GUMROAD_LAUNCH_PLAN.md` — documentación técnica completa de esta fase.

### Corregido (bug real, encontrado durante la verificación E2E de esta fase)
- **Los launchers apuntaban a una carpeta `app/` que nunca existió en el paquete real** — escritos originalmente asumiendo una estructura de paquete distinta a la que terminó implementando `build-commercial.mjs` (que copia `index.html`/`assets/` directamente a la raíz del paquete, junto a los propios launchers — ver Packaging Guide). Encontrado corriendo el launcher REAL contra el paquete REAL producido por `pnpm build:commercial` (no solo sirviendo la carpeta manualmente con `python -m http.server`, que había enmascarado el bug en verificaciones anteriores de esta misma fase) — el script fallaba de inmediato con "No se encontró la carpeta 'app'". Corregido en ambos launchers (`.command`/`.bat`) para servir su propio directorio; re-verificado con el launcher real + Playwright/Chromium real contra el `.zip` reconstruido: carga sin errores, servidor se apaga limpio al terminar el proceso (sin huérfanos).
- `playwright.config.ts` no sembraba `localStorage` — la bienvenida de primera ejecución (nueva en esta fase) aparecía en cada test E2E y bloqueaba todo click contra la app (`welcome-dialog-overlay` interceptando eventos de puntero), causando 46 fallos reales en la suite completa. Corregido con `use.storageState` marcando la bienvenida como ya vista para todos los specs existentes (ninguno prueba la bienvenida en sí). Re-verificado: 51/51 tests en 38s (antes fallaba en 9-18 min de reintentos).
- El script de build inicialmente estampaba `buildMetadata` (con un `builtAt` real y variable) en `commercial-product.json` ANTES de compilar — como el manifest se importa de forma estática, esto hacía que el bundle de JS cambiara de hash en cada build aun para el mismo commit (reproducibilidad rota, detectado corriendo el build dos veces y comparando). Corregido: el manifest se compila siempre sin modificar; la metadata real de build se escribe después, solo en los archivos sueltos del paquete (`commercial-product.json`/`version.json`), nunca en el bundle.

### Fuera de alcance (deliberado, ver la autorización de Fase 4.2)
Backend, Supabase, autenticación, cuentas, suscripciones, Stripe, checkout propio, validación de licencia por clave, activación online, DRM, límites de dispositivo, panel de administración, sincronización en la nube, auto-actualizador, marketplace, sistema de afiliados, telemetría de producción, Mockup Engine, nuevos Builders, sistema de bundles completo, protección anti-copia absoluta. Publicación real en Gumroad queda para una fase posterior con autorización explícita — este release deja el `.zip`/checklist listos, sin publicar.
