...
- **"Exportar para impresión" no permite editar el nombre de archivo** dentro del wizard — usa directamente el nombre del Project al momento de abrir el diálogo (ver UX Audit 0008).
- **Los issues de Preflight de impresión no se pueden localizar visualmente en el Production Preview** — el paso de Preflight es una lista de texto plano, sin interacción hacia el paso de preview (ver UX Audit 0008).
...
- **Solo un perfil de impresión imposicionable ("Sticker Sheet")** — el paso de perfil está preparado estructuralmente para más de una opción, pero solo existe una hoy.
- **Sin UI de asignación de `metadata.role: "die-line"`** en el Inspector — un `Project` recién creado sin ese rol asignado siempre bloquea en Preflight (`cut_path_missing`) con el perfil por defecto; brecha del editor en general, no de esta fase, pero es la primera fase que depende de ella para su camino feliz (ver UX Audit 0008).
