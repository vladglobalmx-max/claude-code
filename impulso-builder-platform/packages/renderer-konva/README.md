...
> FOUNDATION 3 (base) + EDITOR 2 (selección) + EDITOR 3 (movimiento) + EDITOR EPIC 1 (manipulación) + Sticker Creation Experience (grupos, edición de texto) + Export Engine Foundation (Stage headless para PNG, Epic 3) + Print Engine Raster Pipeline (extensión aditiva del Stage headless para sangrado/escala/filtrado, Epic 9 / Fase 9.2) de Impulso Builder Platform. El primer `RendererAdapter` concreto: traduce Document Schema → Scene Graph → Konva, y eventos de Konva → llamadas al Engine. El único paquete de la plataforma que depende de Konva. Ver [ADR-0004](../../docs/adr/0004-renderer-adapter.md) (base), [ADR-0006](../../docs/adr/0006-selection-system.md) (selección), [ADR-0007](../../docs/adr/0007-transform-system.md) (movimiento), [ADR-0008](../../docs/adr/0008-manipulation-system.md) (resize/rotate/handles), [ADR-0010](../../docs/adr/0010-sticker-creation-experience.md) (grupos, texto editable), [ADR-0012](../../docs/adr/0012-export-engine.md) (Stage headless) y [ADR-0022](../../docs/adr/0022-print-engine-raster-pipeline.md) (extensión de impresión física) para el razonamiento completo, y [PERFORMANCE_BUDGET.md](../../docs/PERFORMANCE_BUDGET.md) para el análisis de rendimiento.

**Estado:** completo, incluyendo selección visual (Editor 2), movimiento por arrastre (Editor 3), el sistema completo de manipulación (Editor Epic 1), grupos que actúan como una sola unidad seleccionable/arrastrable más edición de texto in-canvas (Sticker Creation Experience), un Stage headless reutilizado por `@impulso/export-engine` para rasterizar PNG (Epic 3), y — de forma aditiva, Fase 9.2 — soporte para sangrado/escala/filtrado en el mismo Stage headless, consumido por `@impulso/print-engine`. No implementa Canvas UI, Toolbar, Sidebar, Zoom, Pan, Inspector, Layers Panel ni la exportación en sí (el string SVG/la orquestación de export viven en `@impulso/export-engine`/`@impulso/print-engine`) — eso es alcance de la app o de esos paquetes, no de este.
...
└── src/
    ...
    (229 tests, ~99% de statements/lines — ver "Riesgos")
```
...
### 3.9b.1 Extensión aditiva para impresión física (`OffscreenRenderOptions`, Épica 9 / Fase 9.2)

`renderPageToStage` gana 4 opciones nuevas, todas opcionales y sin efecto alguno cuando no se usan — el comportamiento existente (Epic 3, `@impulso/export-engine`) queda intacto byte a byte:

- **`canvasSizePx`/`contentOffsetPx`**: permiten pedir un Stage MÁS GRANDE que la página (el BleedBox de `@impulso/print-engine`, nunca solo el TrimBox) y desplazar el contenido dentro de ese lienzo extendido sin mover ningún `SceneObject` real — el desplazamiento se aplica a un `Konva.Group` contenedor que envuelve todo el contenido, nunca a los objects individualmente. Confirma para Fase 9.2 lo que ya se había verificado para Fase 9.1 (ADR-0021): ni el Stage ni ningún Layer/Group aplican `clip`/`clipFunc`, así que un object cuya geometría cruza el borde del TrimBox se rasteriza completo dentro del BleedBox, sin recortarse.
- **`contentScale`**: aplica `PrintJob.scale` (que hasta esta fase se validaba en Preflight pero nunca llegaba a ninguna salida real) al mismo `Konva.Group` contenedor — `scaleX`/`scaleY`, aplicado DESPUÉS de la traslación de `contentOffsetPx` (el orden de Konva: primero mueve el Group, luego escala desde su nuevo origen) — así el escalado queda anclado exactamente en el origen del TrimBox dentro del BleedBox, no en la esquina del canvas extendido. Verificado con una prueba de matemática exacta: un rect en (10,10) local, con offset (8,8) y `contentScale=2`, aparece en (28,28) absoluto — `8 + 10×2`, nunca `(8+10)×2`.
- **`shouldRenderObject(object, context)`**: predicado de filtrado, deliberadamente más flexible que un solo `excludeMetadataRole` — permite decisiones basadas en cualquier campo del object o del contexto (página, ancestro, profundidad). El filtrado es recursivo y coherente en groups mixtos (hijos filtrados y no filtrados conviven, nunca se filtra "todo o nada" por tener un hijo excluido) y nunca deja un `Konva.Group` vacío innecesario cuando TODOS sus hijos fueron excluidos. El orden visual relativo entre hermanos se preserva siempre. La medición (`getClientRect`/`getSelfRect`) y el recorrido de construcción del árbol usan el MISMO filtro — nunca hay una vista "medida" distinta de la vista "dibujada".
- Sin ninguna opción nueva, `renderPageToStage` produce el árbol Konva idéntico al de Epic 3/Fase 9.1 — verificado explícitamente por tests dedicados, no solo por inspección. Estas 4 opciones tampoco afectan jamás el recorte/interacción del editor visible (`renderer.ts`/`mount`) — son exclusivas del pipeline offscreen.

Consumido por `@impulso/print-engine`'s `renderPrintPage` (Fase 9.2) para construir el raster físico con sangrado real — ver [ADR-0022](../../docs/adr/0022-print-engine-raster-pipeline.md) para el contrato completo del lado del Print Engine.
