...
- **Un solo perfil imposicionable ("Sticker Sheet")** — el paso de perfil está preparado estructuralmente para más de una opción, pero solo existe una hoy.
...
- Fase 9.5 (Hardening & Golden Tests) puede sumar golden fixtures de imposición y una revisión de presupuesto de memoria con evidencia real de dispositivos, pero no modifica este flujo de producto salvo que se descubra un defecto real durante ese trabajo.
- Una UI de asignación de die-line en el Inspector, más perfiles imposicionables, y la localización de issues de Preflight en el preview quedan registrados en el backlog (ver `UX_BACKLOG.md`) para una decisión de producto futura — ninguno se implementó en esta fase por estar fuera de su alcance aprobado.
