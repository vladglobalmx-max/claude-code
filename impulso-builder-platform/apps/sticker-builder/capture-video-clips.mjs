...
// Fase 5 - animatic: una sola grabacion continua y real de la app en
// ejecucion, cubriendo los 7 pasos del flujo de trabajo (Escenas 3-4 del
// storyboard). Se recorta en clips individuales despues con ffmpeg, usando
// las marcas de tiempo que este script imprime en consola. Nada de esto es
// una interfaz simulada: es Chromium real controlando la app real en
// localhost:4174.

const OUT = '/tmp/claude-0/-home-user-claude-code/b61b8831-19e6-5860-8768-b87ff9b98d8c/scratchpad/video';
const VIDEO_DIR = `${OUT}/raw-recording`;
...
async function insertAndStyle({ content, fontSize, x, y }) {
...
await insertAndStyle({ content: 'VELARA', fontSize: 30 });
...
// segundo elemento: una imagen (un Text sin `size` explícito no participa
// en el chequeo de safe area - limitación conocida y documentada del
// motor). Se sube un PNG pequeño generado por el propio <canvas> del
// navegador y se posiciona a caballo del borde de la página (mitad dentro,
// mitad fuera) para que el chequeo real de safe area lo detecte como
// "cruza el límite" - un error de novato genuino, no un valor fabricado.
// 480px nativos para un footprint de 40mm -> ~305 PPI efectivo (por encima
// del umbral de 300 PPI) para que el UNICO problema real demostrado sea el
// cruce de safe area, no tambien una resolucion baja incidental.
...
await page.waitForTimeout(200);
const widthMm = 40;
const straddleX = -(widthMm / 2);
const xImgField = inspectorImg.getByLabel('X').first();
await xImgField.fill(String(straddleX.toFixed(1)));
...
await page.waitForTimeout(400);
mark(`paso3-disenar-fin (imagen a caballo del borde, X=${straddleX.toFixed(1)}mm, ancho=${widthMm.toFixed(1)}mm, a proposito)`);

// ---- PASO 4/6: exportar para impresion -> preview con sangrado/marcas ----
...
mark('paso4-exportar-preview-fin');

// ---- PASO 5: preflight detecta un problema real ----
mark('paso5-preflight-problema-inicio');
...
// ---- PASO 7: imposicion en hoja (volver a preview para verla con foco en imposicion) ----
