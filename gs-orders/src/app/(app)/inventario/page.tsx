...
import type { InventoryIncomingByProduct, InventoryStockLevel, Warehouse } from "@/types/domain";
 ...
 * ON HAND/INCOMING se DERIVAN vía RPC (rpc_inventory_stock_levels /
 * rpc_inventory_incoming_by_product) — nunca se cachean aquí. COMMITTED es
 * 0 en esta fase (ver DECISIÓN en 0036_inventory_mvp.sql — sin fuente real
 * de reservas todavía, propuesta para 6N). INCOMING es por producto
 * (todavía no tiene almacén asignado) — se repite en cada fila de almacén
 * de ese producto, documentado en la propia columna.
 */
export default async function InventarioPage({
  ...
  const [{ data: productsData }, { data: businessUnitsData }, { data: warehousesData }, { data: onHandData }, { data: incomingData }] =
    await Promise.all([
      supabase
