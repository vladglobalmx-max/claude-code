...
- `tests/integration/quotation-mutations.test.ts`: resolución de precio (lista del cliente vs. default de línea), folio real emitido atómicamente con el borrador, cálculo de margen/total al agregar y quitar partidas, las dos rutas que disparan `requiresApproval` (margen bajo el mínimo, descuento sobre el límite del vendedor), y las transiciones de envío (`SENT` vs. `PENDING_APPROVAL`).
- `tests/e2e/login.spec.ts`, `tests/e2e/products.spec.ts`, `tests/e2e/product-crud.spec.ts`, `tests/e2e/customers.spec.ts`, `tests/e2e/sequences.spec.ts` y `tests/e2e/quotations.spec.ts`: navegador real — contraseña incorrecta, redirección sin sesión, RBAC del dashboard/catálogo/clientes/folios/cotizaciones, el flujo completo de alta/edición de producto y de cliente, la emisión de folios de prueba consecutivos, y una cotización real creada, con partidas agregadas y enviada (tanto el caso sano como el que requiere autorización).
