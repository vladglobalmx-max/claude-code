...
import type { CommissionRecord, CommissionRecordStatus, CommissionEvent, SalesOrderCurrency } from "@/types/domain";
...
/**
 * Detalle de una comisión (THÖREN 0073; vista propia de vendedor, 0083,
 * ajuste post-review). `canManage` conserva EXACTO el detalle completo de
 * 0073 (SELECT directo sobre commission_records — la policy NO tiene rama
 * de dueño-vendedor, sin cambios). La rama de vendedor consume
 * EXCLUSIVAMENTE rpc_get_own_commission() (SECURITY DEFINER, 0083): sin
 * SELECT sobre commission_records ni ninguna otra tabla — 0 filas (no
 * excepción) si la comisión no existe o es de otro vendedor -> 404, mismo
 * criterio que /facturas/[id]. Sin historial (commission_events no se toca
 * en 0083), sin PDF, sin CommissionStatusActions.
 */
export default async function ComisionDetailPage({ params }: { params: { id: string } }) {
    ...
    return (
      <div className="mx-auto max-w-3xl px-6 py-8">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          ...
          <DownloadPdfButton docType="commission" id={record.id} />
        </div>
  ...
  // false. Consume EXCLUSIVAMENTE rpc_get_own_commission() (SECURITY
  // DEFINER): sin SELECT sobre commission_records ni ninguna otra tabla —
  // el RPC ya trae folio/cliente resueltos. 0 filas (comisión inexistente
  // o de otro vendedor) -> 404, sin distinguir el motivo (no-disclosure).
  // ===========================================================================
  const { data: ownData } = await supabase.rpc("rpc_get_own_commission", { p_commission_id: params.id });
  const record = ownData?.[0];
  if (!record) notFound();
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-8">
      ...
      <Card>
        <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <p className="text-xs font-medium uppercase tracking-wide text-ink-faint">Tu comisión</p>
            <p className="text-2xl font-bold text-ink">{formatMoneyByCurrency(record.commission_amount, record.currency as SalesOrderCurrency)}</p>
          </div>
          <StatusBadge status={record.status as CommissionRecordStatus} labels={COMMISSION_RECORD_STATUS_LABELS} variants={COMMISSION_RECORD_STATUS_BADGE} className="text-sm" />
        </CardHeader>
        <CardContent>
          <dl className="grid grid-cols-2 gap-x-6 gap-y-3 rounded-xl border border-border bg-surface-2/50 p-4">
            <div>
              <dt className="text-xs text-ink-faint">Sales Order</dt>
              <dd className="text-sm font-medium text-ink">
                <Link href={`/ordenes-venta/${record.sales_order_id}`} className="font-mono text-accent hover:underline">
                  {record.sales_order_folio}
                </Link>
              </dd>
            </div>
            <div>
              <dt className="text-xs text-ink-faint">Cliente</dt>
              <dd className="text-sm font-medium text-ink">{record.customer_name ?? "—"}</dd>
            </div>
            <div>
              <dt className="text-xs text-ink-faint">Fecha de generación</dt>
              <dd className="text-sm font-medium text-ink">{formatDateShort(record.created_at)}</dd>
            </div>
          </dl>
        </CardContent>
      </Card>
