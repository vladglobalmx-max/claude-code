...
| **⚠️ SEVERIDAD ALTA — El handle de rotación queda fuera del área interactiva del Stage cuando el object está a menos de 24px (`ROTATE_HANDLE_OFFSET`, `handles.ts`) del borde superior de la página** | Un `<canvas>` HTML nunca renderiza ni recibe hit-testing fuera de sus propias dimensiones; el Stage de Konva mide exactamente `page.size` (`stage.width/height`). El handle de rotación se dibuja `ROTATE_HANDLE_OFFSET` (24px) arriba del borde superior del object — si ese borde está a menos de 24px del borde superior de la página, el handle cae en coordenadas Y negativas del Stage: invisible e inalcanzable con el mouse. **Impacto práctico**: el preset built-in "Sticker circular" (y "cuadrado"/"rectangular") posiciona sus objects en `Y=0` — es decir, TODO sticker circular nuevo tiene su línea de corte imposible de rotar arrastrando el handle hasta que el usuario la mueva hacia abajo manualmente primero. No es un bug nuevo de Fase 7.1/7.2/7.3 (el handle de rotación existe desde Editor Epic 1/ADR-0008) pero nunca había sido detectado. No se corrigió en Fase 7.3.5 porque una solución correcta para CUALQUIER rotación requiere una decisión de arquitectura (¿el Stage debería tener margen alrededor de la página para los handles? eso cambia el sistema de coordenadas usado en todo `renderer-konva`, incluyendo el Stage headless de exportación) — un simple clamp de la posición solo resuelve el caso sin rotación, y aplicarlo de forma incompleta podría verse como un bug distinto. | Fase 7.3.5 (Beta Stabilization), Regression Audit — reproducido en Chromium con el preset "Sticker circular" recién creado (sin ninguna interacción previa del usuario). |
