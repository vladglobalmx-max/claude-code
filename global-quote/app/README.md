...
- `tests/e2e/login.spec.ts`, `tests/e2e/products.spec.ts`, `tests/e2e/product-crud.spec.ts`, `tests/e2e/customers.spec.ts` y `tests/e2e/sequences.spec.ts`: navegador real — contraseña incorrecta, redirección sin sesión, RBAC del dashboard/catálogo/clientes/folios, el flujo completo de alta/edición de producto y de cliente, y la emisión de folios de prueba consecutivos.
