...
> Print Engine de plataforma, nacido en la Épica 9 (Professional Print Engine). Produce salidas confiables para impresión digital, corte y producción comercial — modelo de `PrintJob`, unidades físicas, boxes de PDF (Trim/Bleed/Media/Safe Area), Preflight, el pipeline real de raster (PNG físico multipágina y PDF aplanado de alta resolución), marcas de corte vectoriales/safe area verificable/cut paths reales (Fase 9.3), y ahora (Fase 9.4) imposición/repetición en hojas con reutilización real de raster. Reutilizable por cualquier Builder futuro. Ver [ADR-0021](../../docs/adr/0021-print-engine-foundation.md) (Fase 9.1), [ADR-0022](../../docs/adr/0022-print-engine-raster-pipeline.md) (Fase 9.2), [ADR-0023](../../docs/adr/0023-print-engine-marks-safearea-cutpaths.md) (Fase 9.3) y [ADR-0024](../../docs/adr/0024-print-engine-imposition.md) (Fase 9.4).

**Estado:** Fase 9.4 (Imposition & Sheet Repetition) completa, con su UI real de producto ya construida en `apps/sticker-builder` (ver [ADR-0025](../../docs/adr/0025-production-export-workflow.md)). Deliberadamente **sin** nesting irregular, sin optimización automática de desperdicio, sin tiling de gran formato, sin integración con RIP/plotter todavía — eso queda para una fase futura de la misma épica, sin autorización todavía.
...
## 8. Mejoras futuras (fase ya planificada de la Épica 9)

- **Fase 9.5** — Hardening & Golden Tests: presupuestos, memoria, determinismo, infraestructura completa de golden files (sobre `testUtils/goldenFixtures.ts`, incluyendo escenarios de imposición), validación programática de PDF, E2E completo. Sin autorización todavía — no se inicia hasta que se apruebe explícitamente.
