...
Arquitectura aprobada: Impulso Engine nunca depende de una librería de render. Todo proyecto se representa en un **Document Schema** propio, y el renderer (Konva, hoy) es un adaptador reemplazable — `Document Schema → Engine → Renderer → Konva`. Ver el diseño técnico completo en [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) y el mapa de arquitectura de producto/módulos vigente en [`docs/product/THOREN_TECHNICAL_ARCHITECTURE.md`](docs/product/THOREN_TECHNICAL_ARCHITECTURE.md) (el `03-Architecture-Map.md` de la plataforma pre-reinvención queda archivado en [`docs/archive/pre-reinvention-platform/`](docs/archive/pre-reinvention-platform/03-Architecture-Map.md)).
...
| EPIC 5 — Project Library / Workspace | [`packages/storage-kit`](packages/storage-kit) + [`packages/project-library`](packages/project-library) + [`apps/sticker-builder`](apps/sticker-builder) | ✅ Completo — ver [ADR-0014](docs/adr/0014-project-library-workspace.md) |
