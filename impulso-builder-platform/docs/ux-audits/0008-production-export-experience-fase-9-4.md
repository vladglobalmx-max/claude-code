...
- ~~El paso de perfil solo puede mostrar el perfil "Sticker Sheet"~~ — **corregido en Fase 9.5**: esta auditoría diagnosticó mal la causa — no era una limitación del motor (`PRINT_PROFILES` siempre tuvo 4 perfiles completos), sino un `pendingProfileId` hardcodeado en la UI que nunca se conectó a un selector real. Ver la enmienda de Fase 9.5 en ADR-0025 para el detalle completo, incluida la decisión explícita de dejar "Web Preview" fuera de este wizard.
